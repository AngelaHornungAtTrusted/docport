...
class Popup {
    ...
    init() {
        (this.id = this.params.id ?? "popup"),
            ...
            (this.css += `\n        .popup.${this.id} {\n            transition-duration: ${this.fadeTime};\n            text-shadow: ${this.textShadow};\n            font-family: '${
                ...
                this.fixedHeight ? this.height : "unset"
        ...
        for (let t = 0; t < this.content.length; t++) {
            ...
            if ("" !== i) {
                ...
                for (i = i.replace(/  /g, "&nbsp;&nbsp;"); /{a-(.*?)}\[(.*?)]/.test(i); ) i = i.replace(/{a-(.*?)}\[(.*?)]/g, '<a href="$1" target="_blank">$2</a>');
                for (; /{btn-(.*?)}\[(.*?)]/.test(i); ) i = i.replace(/{btn-(.*?)}\[(.*?)]/g, '<button class="$1">$2</button>');
                (i = i
    ...
    show() {
        this.popupEl.classList.remove("fade-out"), this.popupEl.classList.add("fade-in"), postShow(this.params.disableScroll ?? !0);
    }
    hide() {
        this.popupEl.classList.remove("fade-in"), this.popupEl.classList.add("fade-out"), localStorage && this.showOnce && localStorage.setItem("popup-" + this.id, !0), postHide(this);
    }
}
function postShow(t) {
    t && disableScroll();
